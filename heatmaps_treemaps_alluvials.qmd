...
::: callout-important
## Never use the na.omit() or drop_na() functions on an entire dataset!
:::

```{r}
flights_nona1 <- na.omit(flights)
```
...
```{r}
# convert pivot_longer
flights_long <- by_dest |>
...
```{r}
ggplot(flights_long, aes(x=metric_type, y=dest, fill = metric_value))+
  geom_tile()
```
...
flights3 <- flights2 |>
  group_by(month_label)|>  # name = name of the airline
