...
gapminder1 <- gapminder |>
  mutate(group = case_when(
    region %in% c("Western Europe","Northern Europe","Southern Europe",
          "Northern America","Australia and New Zealand") ~ "The West",
...
```{r}
ggplot(gapminder2, aes(x=fertility, y=life_expectancy, color = group, size = population_in_millions)) +
...
```{r}
ggplot(df2, aes(year, state,  fill = rate)) +
  geom_tile(color = "grey50") +
  scale_x_continuous(expand=c(0,0)) +
  scale_fill_gradientn(colors = brewer.pal(9, "Reds"), trans = "sqrt") +
  geom_vline(xintercept=1963, col = "blue", size = 1.5) +
  theme_minimal() +
